...
let actionMovies = movies.filter((movie) => movie.genre === "Action").forEach((movie) => console.log(movie.title));

//What movies were bad. Bad will be defined as a rating of less than 7.
let badMovies = movies.filter((movie) => movie.rating < 7).forEach((movie) => console.log(movie.title));

//What movies were good. Good will be defined as a rating of 8 or higher?
let goodMovies = movies.filter((movie) => movie.rating >= 8).forEach((movie) => console.log(movie.title));

//What movie was directed by Wes Anderson?
let movieByWes = movies.find((movie) => movie.director === "Wes Anderson");
console.log(movieByWes.title);

// What are the comedy movies in the list?
let comedies = movies.filter((movie) => movie.genre === "Comedy").forEach((movie) => console.log(movie.title));

//Create an array of strings with just the names of the directors.
directorsArr = [];
movies.forEach((movie) => directorsArr.push(movie.director));
...
//   movies.forEach(movie => console.log(`https://www.imdb.com/title/${movie.imdbID}/`));

//Create an array of strings with the urls from the last task
let imdbArr = [];
movies.forEach((movie) => imdbArr.push(`https://www.imdb.com/title/${movie.imdbID}/`));
console.log(imdbArr);

//Bonus: What was the total budget for all action movies in 2021?
let budgetIn2021 = movies
  .filter((movie) => movie.year === 2021)
  .reduce((acc, movie) => acc + movie.budgetInMillions, 0);
console.log(budgetIn2021);
