...
let mySong = songs.find((song) => song.songID === 10);
...
let popSongsArr = songs.filter((allSongs) => allSongs.genre === "Pop");
console.log(popSongsArr.length);
...
let songTitles = popSongsArr.map((song) => song.title);
...
let totalDuration = songs.reduce((durationTotal, song) => durationTotal + song.durationInSeconds, 0);
...
let songsByOlivia = songs.filter((allSongs) => allSongs.artist === "Olivia Rodrigo");
let titles = songsByOlivia.map((song) => song.title);
console.log(titles);
...
let longestSong = songs.reduce((acc, val) => {
  acc = val.durationInSeconds > acc.durationInSeconds ? val : acc;
  return acc;
});
...
let stringsArr = songs.map((song) => `${song.title} by ${song.artist}`);
...
let songAvgDuration = songs.reduce((durationTotal, song) => durationTotal + song.durationInSeconds, 0);
let avg = songAvgDuration / songs.length;
console.log(avg);
...
let songs2021 = songs.filter((allSongs) => allSongs.year === 2021);
console.log(songs2021.length);
...
let songRB = songs.find((song) => song.genre === "R&B");
console.log(songRB.title);
